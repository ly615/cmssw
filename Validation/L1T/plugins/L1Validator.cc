...
void L1Validator::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  */ 
  ...
  for(uint i=0; i < GenJets->size(); i++){
     ...
     _Hists.Fill(L1ValidatorHists::Type::Jet, GenPart, RecoPart);
  }
  ...
  for(uint i=0; i < GenParticles->size(); i++){
    ...
    if(status==1 && abs(pdg)==13){  //Muon
       ...
       _Hists.Fill(L1ValidatorHists::Type::Egamma, GenPart, RecoPart);
       ...
       _Hists.Fill(L1ValidatorHists::Type::Tau, GenPart, RecoPart);
